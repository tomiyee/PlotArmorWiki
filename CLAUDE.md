...
/{serial}                   # serial home page (metadata + home wiki page content)
/{serial}/new               # new wiki page creation form
/{serial}/{page-slug}       # wiki page
```
...
- `src/app/[serial]/page.tsx` — serial home page; resolves serial via `WHERE slug = ?`, fetches home page content (sections, infobox, child pages at cutoff), and renders `<SerialMetadataEditor>` + `<PageEditor>` inside a two-column layout with `<SerialTOCSidebar>` on the left.
- `src/app/[serial]/actions.ts` — Server Actions for volume/chapter CRUD (`addVolume`, `addChapter`, `deleteVolume`, `deleteChapter`, `renameVolume`, `renameChapter`, `updateSerialTypes`, `reorderVolumes`, `reorderAllChapters`) and serial metadata (`updateSerialMetadata`). Reorder actions reassign `chapters.idx` — no post-reorder version repair is needed because content revisions are keyed by `chapter_id` and follow their chapter's new position automatically.
- `src/app/[serial]/new/page.tsx` — page creation form; collects page name, intro chapter (grouped by volume via optgroup), and required parent page (dropdown of all existing pages in the serial); pre-selects a parent when `?parentPageId=` is in the query string.
- `src/app/[serial]/new/actions.ts` — `createPage` Server Action; generates a unique slug (appends `-2`, `-3`, … on collision), inserts into `pages`, seeds an initial `page_titles` row, inserts a `page_relationships` row linking to the required parent at the head chapter, then redirects to `/{serial}/{page-slug}`.
- `src/app/[serial]/layout.tsx` — serial-scoped nested layout; fetches the serial, volumes, chapters, and immediate children of the home page (for the "Pages" navbar dropdown), then injects them + `<ChapterSelector>` + `<SerialTOCDrawer>` into the navbar via `<SerialNavInjector>`.
- `src/app/[serial]/[page]/page.tsx` — wiki page view; resolves serial/page by slug, reads the user's chapter cutoff from cookie, fetches chapter-filtered section content, infobox data, and child pages (all using the max-idx subquery-join pattern), fetches all chapters and head chapter id for the edit-mode selector, fetches wiki pages visible at the user's cutoff for autocomplete, then delegates all rendering to `<PageEditor>`. Redirects to `/{serial}` if the page is the home page.
- `src/app/[serial]/[page]/PageEditor.tsx` — Client Component that owns the page body layout. In read mode renders sections, infobox, and a child-pages list (links to `/{serial}/{slug}` with `?parentPageId=` on the "New page" link). In edit mode each section gets a `<WikiLinkMDEditor>` (SSR disabled), floater fields get text inputs, and a "Writing as of:" `<Select>` lets the editor target any chapter. Changing the chapter calls `getPageContentAtChapter` to pre-fill drafts; on save, calls `savePageContent` with the selected `targetChapterId` then refreshes.
- `src/app/[serial]/[page]/actions.ts` — `savePageContent` Server Action and `getPageContentAtChapter` Server Action. `savePageContent` resolves serial/page by slug and upserts each section/floater field at `(pageId, sectionId, chapterId)` — uses the optional `targetChapterId` when provided, otherwise defaults to the head chapter. `getPageContentAtChapter` runs the same max-idx subquery join but against an explicit chapter id instead of the cookie, returning `{ sections, floaterImageUrl, floaterRows }` for the edit-mode chapter selector.
...
- `src/components/CategoryManager.tsx` — legacy Client Component for managing page categories; no longer wired into any route after the DAG migration.
...
- `src/components/SerialNavInjector.tsx` — Client Component (renders null) that injects serial data, `<ChapterSelector>`, and `<SerialTOCDrawer>` into the global navbar context via `useLayoutEffect`. Mounted inside the serial layout as a sibling of page content.
