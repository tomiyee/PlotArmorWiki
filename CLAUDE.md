...
- `src/app/[serial]/[schema]/[page]/actions.ts` — `savePageContent` Server Action; resolves serial/schema/page by slug/name, upserts each section/floater field at `(pageId, sectionId, chapterId)` — uses the optional `targetChapterId` parameter when provided, otherwise defaults to the head chapter. Readers at an earlier cutoff see the prior version via the max-idx read path.
