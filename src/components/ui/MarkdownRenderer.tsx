 ...
 * (those generated by `remarkWikiLinks`) with `<WikiLinkPreview>` hover cards.
 *
 * A wiki link generated by the remark plugin always has an href of the form
 * `/{serialSlug}/{encodedPage}` (2-level URL — no category segment), so we
 * detect those by prefix and extract the page slug for the preview fetch.
 */
function makeAnchorComponent(serialSlug: string): Components["a"] {
  ...
  return function WikiAnchor({ href, children }) {
    if (href && href.startsWith(prefix)) {
      ...
      // Only treat as a wiki link if there is no further slash (2-level URL).
 ...
 * converted to clickable links with hover card previews. Links inside
 * backticks are left as-is.
 *
 * @example
