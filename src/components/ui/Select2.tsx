 ...
 * />
 */
function Select2<T>(props: Select2Props<T>) {
  const {
    options,
    value,
    onChange,
  ...
  const allRowIds = useMemo(() => {
    function collectIds(opts: Option<T>[], prefix = ""): string[] {
      return opts.flatMap((opt, i) => {
        ...
        return [
          id,
          ...(opt.children ? collectIds(opt.children, id) : []),
        ];
      });
    }
      ...
      <div
        ...
        className={cn(
          ...
          option.disabled
            ...
            : "hover:bg-muted/40",
          isActive && !option.disabled && "bg-primary/10 font-medium text-primary",
        )}
        onMouseDown={(e) => {
          e.preventDefault();
  ...
  const popupWidth =
    widthMode === "trigger" ? "var(--anchor-width)" : "200px";
