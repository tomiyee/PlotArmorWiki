 ...
 * `/{serialSlug}/{encodedCategory}/{encodedPage}`, so we detect those by prefix
 * and extract the category + page names for the preview fetch.
 */
function makeAnchorComponent(serialSlug: string): Components["a"] {
  ...
  return function WikiAnchor({ href, children }) {
    if (href && href.startsWith(prefix)) {
      ...
      if (slashIdx !== -1) {
        const categoryName = decodeURIComponent(rest.slice(0, slashIdx));
        const pageName = decodeURIComponent(rest.slice(slashIdx + 1));
        return (
          <WikiLinkPreview
            href={href}
